...
- Admin generation rows may include `canCancel=true` for queued jobs and for `ProviderQueued`/`ProviderProcessing` FAL jobs only when the persisted provider request resolves to a trusted cancellation endpoint. `POST /api/admin/templates/generations/{generationId}/cancel` is `AdminOnly`. Queued cancellation remains synchronous and returns HTTP 200. Provider cancellation first durably transitions the job to `CancellationRequested`, returns HTTP 202 with the unchanged generation response shape, and is delivered by the worker with bounded retries. The worker refunds at most once only after FAL accepts cancellation; duplicate requests must not create duplicate provider calls or refunds. An unavailable or untrusted provider cancellation target returns HTTP 409 ProblemDetails with `templates.generation_cancel_provider_unsupported`. Other non-cancellable states return `templates.generation_cancel_not_allowed`. Provider callbacks received while cancellation is pending or after local cancellation are acknowledged and ignored so they cannot resurrect or import a cancelled result.
- Admin generation list status values remain `Pending`, `Running`, `Completed`, `Failed`, `Cancelled`, and `Retrying`, with additive `Cancelling` for `CancellationRequested`. `Running` includes `Processing`, `SubmittingToProvider`, `ProviderQueued`, `ProviderProcessing`, and `ImportingMedia`. Dashboard metrics add the `cancellingJobs` count without changing existing fields.
...
- Response `status` is a public string contract, not a numeric enum. Current wire values include `Queued`, `SubmittingToProvider`, `ProviderQueued`, `ProviderProcessing`, `ImportingMedia`, `CancellationRequested`, `Completed`, `Failed`, and `Cancelled`; clients must never receive numeric enum values such as `"8"`, `"9"`, `"10"`, or `"11"` for provider queue/import/cancellation stages.
