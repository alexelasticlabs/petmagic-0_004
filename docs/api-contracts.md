...
- Legacy mobile receives category ordering, enabled sections, selected covers and pinned card ordering immediately. Mobile V2 also consumes localized page/section copy, carousel visibility, autoplay and per-surface flags. It requests `sectionLimit=24` and `itemsPerSection=12`, matching the configured maximum; server limits still apply. Deploy backend/schema before admin; mobile may be updated independently.
- On the current mobile toolbar, V2 `searchEnabled=true` opens the catalog with search focused; `false` retains the normal All templates entry. Random selection and category navigation remain available. A response without V2 page settings retains the existing catalog toolbar. More/hero navigation continues to send the original `category` string, never the localized display `title` or `categoryId`.
- Mobile V2 preserves page settings and section metadata in locale-isolated discovery snapshots. Absent or unknown `schemaVersion` uses the legacy layout; invalid optional values fall back safely, and autoplay intervals are clamped to 5–30 seconds. An empty page subtitle hides it. Autoplay remains paused for reduced motion, user interaction, hidden carousel and app background.
- Published `templates.feed.invalidated` events (`scope=full`, `reason=discovery_published`) refresh mobile discovery with a 350 ms debounce independently of catalog version. Superseded responses cannot replace newer configuration; hidden/offline screens defer network work until resume/reconnect and offline launch can restore cached configuration. Pull-to-refresh and the existing 45-second freshness check remain the fallback when realtime is unavailable.
