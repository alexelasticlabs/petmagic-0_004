    ...
    ILogger<PremiumSubscriptionPlansHealthCheck>? logger = null) : IHealthCheck
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            logger?.LogWarning(
                "Premium subscription plan health check found configuration drift. MissingPlanCodes={MissingPlanCodes} InactivePlanCodes={InactivePlanCodes} ProductIdMismatches={ProductIdMismatches}",
                missingPlanCodes,
                inactivePlanCodes,
                productIdMismatches);
            ...
            return HealthCheckResult.Unhealthy(
                "Premium subscription plans are missing, inactive, or do not match the expected provider catalog.",
                data: diagnosticData);
        }
