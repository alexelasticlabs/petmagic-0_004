  ...
  return (
    <AdminPage className={styles.page}>
      <AdminPageHero
        ...
        actions={<Link href={`/${locale}/users`} className={styles.backLink}>{text.navUsers}</Link>}
        metaItems={metaItems}
      />
        ...
        <div className={styles.profileHeader}>
          ...
          <div className={styles.profileCopy}>
            ...
            <div className={styles.profileBadges}>
              ...
              {user.roles.map((role) => <AdminBadge key={role}>{role}</AdminBadge>)}
            </div>
          </div>
          <div className={styles.profileMeta}>
            <Metric label={text.createdAtLabel} value={formatDateTime(user.createdAtUtc, locale)} />
            <Metric label={text.lastPurchaseLabel} value={formatDateTime(analytics.summary.lastPurchaseAtUtc, locale)} />
            <Metric label={text.lastGenerationLabel} value={formatDateTime(analytics.summary.lastGenerationAtUtc, locale)} />
          </div>
        </div>
      </AdminCard>
            ...
            items={analytics.recentPurchases.map((purchase) => (
              <article key={purchase.orderId} className={styles.dataCard}>
                <div className={styles.dataHeader}>
                  ...
                  <AdminStatusBadge color={purchase.status === "succeeded" ? "#2dd4bf" : "#f59e0b"}>{purchase.status}</AdminStatusBadge>
                </div>
                <p>{purchase.priceAmount} {purchase.currencyCode} • {purchase.paymentProvider}</p>
            ...
            items={analytics.recentGenerations.map((generation) => (
              <article key={generation.generationId} className={styles.dataCard}>
                <div className={styles.dataHeader}>
                  ...
                  <AdminStatusBadge color={generation.status === "Completed" ? "#22c55e" : generation.status === "Failed" ? "#f87171" : "#8da1ba"}>{generation.status}</AdminStatusBadge>
                </div>
                <p>{generation.templateType} • {generation.tokenCost} tokens</p>
          ...
          <DataList
            ...
            items={analytics.recentTemplateEvents.map((event) => (
              <article key={event.eventId} className={styles.dataCard}>
                ...
                <p>{event.source} • {event.deviceClass} • {event.countryCode}</p>
