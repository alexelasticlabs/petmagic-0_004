...
const files = [
  ...
  fileURLToPath(
    new URL("./templates/templates-analytics-hub-page.tsx", import.meta.url)
  ),
  fileURLToPath(
...
describe("admin sensitive display usage", () => {
  ...
  it("sanitizes user identity, role, and generation labels in user-facing admin views", () => {
    const usersManagementSource = readFileSync(files[0], "utf8");
    const userDetailSource = readFileSync(files[1], "utf8");
    const userInlineAnalyticsSource = readFileSync(files[2], "utf8");
