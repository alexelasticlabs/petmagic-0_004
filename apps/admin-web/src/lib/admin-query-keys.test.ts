...
describe("admin-query-keys", () => {
  it("provides stable support inbox root key", () => {
    ...
    expect(adminQueryKeys.supportInbox("Open", "mine")).toEqual(["admin", "support", "inbox", "Open", "mine"]);
  });
  ...
  it("provides explicit disabled keys for users queries", () => {
    ...
    expect(adminQueryKeys.userAnalyticsDisabled).toEqual(["admin", "users", "analytics", "disabled"]);
  });

  it("builds user keys by userId", () => {
    expect(adminQueryKeys.userDetail("user-123")).toEqual(["admin", "users", "user-123", "detail"]);
    expect(adminQueryKeys.userAnalytics("user-123")).toEqual(["admin", "users", "user-123", "analytics"]);
  });
});
