...
void main() {
  ...
  test('google account linking uses the native token flow', () async {
    ...
    final dio = Dio(BaseOptions(baseUrl: 'https://api.petmagic.test'))
      ..httpClientAdapter = _FakeHttpClientAdapter((options) async {
        ...
        expect(options.path, '/api/auth/me/linked-accounts/google/native');
        expect(options.headers['Authorization'], 'Bearer access-token');
        expect(options.data, {'idToken': 'native-google-id-token'});
        return ResponseBody.fromString(
          jsonEncode(const [
            {
              'provider': 'Google',
    ...
    final repository = MobileExternalAuthRepository(
      ...
      appLinks: AppLinks(),
      googleSignInAdapter: _FakeGoogleSignInAdapter(
        (_) async => const GoogleSignInCredential(
    ...
    final accounts = await repository.link(ExternalAuthProvider.google);

    expect(accounts, hasLength(1));
    expect(accounts.single.provider, 'Google');
  });
